/*!
 * Rimbun template behaviour — list renderers, libraries, foliage cover animation.
...
(function () {
  ...
  if (!W || !W.ready) { console.error('[rimbun] core runtime missing'); return; }
  ...
  /* ---------- cover (Swiper fade + drifting dry leaves) ---------- */
  var particles;
      ...
      window.tsParticles.load({
        ...
        options: {
          ...
          particles: {
            ...
            shape: { type: 'image', options: { image: { src: ornBase + 'daun.svg', width: 40, height: 40 } } },
            size: { value: { min: 9, max: 17 } },
  ...
  W.section('gallery', function () {
    ...
    $('#flexbin').innerHTML = photos.map(function (p, i) {
      return '<a href="' + attr(p) + '" class="glightbox" data-gallery="rimbun" aria-label="' + esc(t('gallery.photo', { n: i + 1 })) + '"><img src="' + attr(p) + '" alt="' + esc(t('gallery.photo', { n: i + 1 })) + '" loading="lazy"></a>';
    }).join('');
  ...
  W.section('gift', function () {
    ...
    W.ui.copyButtons($('#kado'));
  });

  /* ---------- turut mengundang / dress code ---------- */
  W.section('turutMengundang', function () {
    $('#inviters').innerHTML = W.ui.inviters();
    if (V.inviters.groom.length && V.inviters.bride.length) $('#inviters').classList.add('two');
  });
  ...
  /* ---------- open: leaves part, medallion grows, cover closes as an iris ---------- */
  var opened = false;
  function openInvitation(instant) {
    if (opened) return; opened = true;
    var btn = $('#openBtn'); btn.style.animation = 'none'; btn.disabled = true;
    var cover = $('#topCover');
    var finish = function () {
      document.documentElement.classList.remove('locked'); document.body.classList.remove('locked');
      window.scrollTo(0, 0);
      if (instant === true) cover.style.display = 'none'; else cover.classList.add('iris');
      W.open();
      $('#musicBtn').classList.add('show');
      W.music.start();
      if (window.gsap && instant !== true) gsap.to('[data-hero]', { opacity: 1, y: 0, duration: 2.2, ease: 'power2.out', stagger: .35, delay: .6 }); else $$('[data-hero]').forEach(function (e) { e.style.opacity = 1; });
      if (window.ScrollTrigger) setTimeout(function () { ScrollTrigger.refresh(); if (aosReady) AOS.refresh(); }, 300);
      setTimeout(function () {
        cover.style.display = 'none'; if (particles) particles.destroy(); if (window.ScrollTrigger) ScrollTrigger.refresh();
    ...
    gsap.timeline({ defaults: { ease: 'power2.inOut' } })
      .to(btn, { autoAlpha: 0, y: 16, duration: .45 }, 0)
      .to('.leaf-l1, .leaf-l2', { xPercent: -90, rotation: '-=18', autoAlpha: 0, duration: 1.2, stagger: .08 }, .1)
      .to('.leaf-r1, .leaf-r2', { xPercent: 90, rotation: '+=18', autoAlpha: 0, duration: 1.2, stagger: .08 }, .1)
      .to('.grove-flowers', { y: 60, autoAlpha: 0, duration: .8 }, .15)
      .to('.cover-head', { y: -30, autoAlpha: 0, duration: .8 }, .25)
      .to('#medallion', { scale: 1.35, duration: 1.1, ease: 'power3.inOut' }, .35)
      .to('#medallion', { autoAlpha: 0, duration: .5 }, 1.1)
      .add(finish, 1.2);
  }
  $('#openBtn').addEventListener('click', function () { openInvitation(); });
  $('#medallion').addEventListener('click', function () { openInvitation(); });

  /* ---------- boot ---------- */
  document.documentElement.classList.add('locked');
  if (/[?&]open=1\b/.test(location.search)) openInvitation(true); // editor preview / screenshots: skip the cover animation
  var loaded = false;
  function ready() {
